...
void TestQgsAttributeForm::testFieldConstraint()
{
  ...
  // set value to 1
  ww->setValues( 1, QVariantMap() );
  QCOMPARE( spy.count(), 1 );
  QCOMPARE( constraintsLabel( &form2, ww )->text(), validLabel );

  // set value to null
  spy.clear();
  ww->setValues( QVariant(), QVariantMap() );
  QCOMPARE( spy.count(), 1 );
...
void TestQgsAttributeForm::testFieldMultiConstraints()
{
  ...
  // change value
  ww0->setValues( 2, QVariantMap() ); // update col0
  ...
  // change value
  spy2.clear();
  ww0->setValues( 1, QVariantMap() ); // update col0
  QCOMPARE( spy2.count(), 1 );
